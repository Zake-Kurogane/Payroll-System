@php
    ...
    $companySub = collect([$company?->company_address, $company?->company_contact, $company?->company_email])
        ->filter()
        ->implode(' • ');
        ...
        <div class="print-page">
            @foreach ($page as $p)
                @php
                    ...
                    $statEE =
                        (float) ($p['sss_ee'] ?? 0) +
                        (float) ($p['philhealth_ee'] ?? 0) +
                        (float) ($p['pagibig_ee'] ?? 0) +
                        (float) ($p['tax'] ?? 0);
                @endphp
                    ...
                    <div class="paystub">
                        <div class="psHead">
                            ...
                            <div class="psHeadRight">
                                <div class="psMetaLine"><span>No</span><strong>{{ $p['payslip_no'] ?? '-' }}</strong>
                                </div>
                                <div class="psMetaLine">
                                    <span>Date</span><strong>{{ $p['generated_at'] ? \Carbon\Carbon::parse($p['generated_at'])->format('m-d-Y') : '-' }}</strong>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="psInfoGrid">
                            <div class="psInfoCol">
                                ...
                                <div class="psInfoRow"><span>Name</span><strong>{{ $p['emp_name'] ?? '-' }}</strong>
                                </div>
                                <div class="psInfoRow"><span>Employee
                                        ID</span><strong>{{ $p['emp_no'] ?? ($p['employee_id'] ?? '-') }}</strong></div>
                                <div class="psInfoRow">
                                    <span>Department</span><strong>{{ $p['department'] ?? '-' }}</strong>
                                </div>
                                <div class="psInfoRow"><span>Position</span><strong>{{ $p['position'] ?? '-' }}</strong>
                                </div>
                                @php
                                    $branchName = $p['external_area'] ?? ($p['area_place'] ?? null);
                                    $branchName =
                                        is_string($branchName) && trim($branchName) !== '' ? trim($branchName) : '-';
                                @endphp
                                <div class="psInfoRow"><span>Assignment</span><strong>{{ $branchName }}</strong></div>
                            </div>
                            <div class="psInfoCol">
                                <div class="psInfoTitle">PAY PERIOD</div>
                                <div class="psInfoRow"><span>Payroll
                                        Month</span><strong>{{ $p['period_month'] ?? '-' }}</strong></div>
                                <div class="psInfoRow"><span>Cutoff</span><strong>{{ $p['cutoff'] ?? '-' }}</strong>
                                </div>
                                ...
                                @endphp
                                <div class="psInfoRow"><span>Pay Method</span><strong>{{ $payMethod }}</strong>
                                </div>
                            </div>
                        </div>
                        ...
                        <table class="psTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>SSS (EE)</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($p['sss_ee'] ?? 0) !!}</td>
                                </tr>
                                <tr>
                                    <td>PhilHealth (EE)</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($p['philhealth_ee'] ?? 0) !!}</td>
                                </tr>
                                <tr>
                                    <td>Withholding Tax</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($p['tax'] ?? 0) !!}</td>
                                </tr>
                                @php
                                    $loanItems = collect($p['loan_items'] ?? [])->filter(
                                        fn($i) => (float) ($i['deducted_amount'] ?? 0) > 0,
                                    );
                                    $sumType = function (string $needle) use ($loanItems) {
                                        return (float) $loanItems
                                            ->filter(
                                                fn($i) => str_contains(
                                                    strtolower((string) ($i['loan_type'] ?? '')),
                                                    $needle,
                                                ),
                                            )
                                        ...
                                        ->filter(
                                            fn($i) => str_contains(
                                                ...
                                                (str_contains(
                                                    ...
                                                    str_contains(strtolower((string) ($i['loan_type'] ?? '')), 'hdmf')),
                                        )
                                        ->sum(fn($i) => (float) ($i['deducted_amount'] ?? 0));
                                ...
                                <tr>
                                    <td>HDMF</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($p['pagibig_ee'] ?? 0) !!}</td>
                                </tr>
                                <tr>
                                    <td>SSS Loan</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($sssLoan) !!}</td>
                                </tr>
                                <tr>
                                    <td>HDMF Loan</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($hdmfLoan) !!}</td>
                                </tr>
                                <tr>
                                    <td>PAGIBIG Housing Loan</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($pagibigHousing) !!}</td>
                                </tr>
                                <tr>
                                    <td>SSS Housing Loan</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($sssHousing) !!}</td>
                                </tr>
                                <tr>
                                    <td>HDMF Calamity Loan</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($hdmfCalamity) !!}</td>
                                </tr>
                                <tr>
                                    <td>Advances</td>
                                    <td class="num">-</td>
                                    <td class="num">{!! $money($advances) !!}</td>
                                </tr>
                                <tr>
                                    <td>Shortages</td>
                                    <td class="num">-</td>
                        ...
                        <div class="psSummaryGrid">
                            <div class="psSummaryBox">
                                <div class="psSummaryLine"><span>Gross
                                        Pay</span><strong>{!! $money($p['gross'] ?? 0) !!}</strong></div>
                                <div class="psSummaryLine"><span>Total
                                        Deductions</span><strong>{!! $money($p['deductions_total'] ?? 0) !!}</strong></div>
                            </div>
